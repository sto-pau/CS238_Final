...
def get_Rewards_States(case_name,time_start,fms_flag,time_analysis):
    ...
    #
    '''
    if time_start == round(time_start): #234
        time_start = round(time_start)
    else:
        time_start = np.round(time_start,1)
	...
	# rewards
        ...
        try: #234
            rewards[t] = data_rewards[data_rewards[:,0]==np.round(time,6),:]
        except:
            print(path_rewards)
            print(time)
    ...
    if fms_flag: #force,moment state
        ...
        for t,time in enumerate(time_analysis):
            try: #234
                states_forces[t] = data_states_forces[data_states_forces[:,0]==np.round(time,6),1:]
                states_moments[t] = data_states_moments[data_states_moments[:,0]==np.round(time,6),1:]
            except:
                print(path_states_forces)
                print(time)
...
if __name__ == '__main__':
    ...
    case_name = 'epsilon_decay0597' #123
    ...
    duration  = 0.3 #total learning length
    sim_step_length = 0.1
    ...
    model = Main.create_model(action_space)
    exploration_policy = Main.EpsilonGreedyExploration(0.5,0.97) # change exploration policy
    ...
    eval_duration  = 0.3 #total learning length
